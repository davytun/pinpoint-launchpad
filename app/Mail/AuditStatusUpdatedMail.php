...
class AuditStatusUpdatedMail extends Mailable
{
    ...
    public function envelope(): Envelope
    {
        ...
        return new Envelope(subject: "Audit Update: {$label}");
    }
    ...
    public function content(): Content
    {
        $body = match ($this->auditStatus) {
            ...
            'complete' => 'Your PARAGON audit is complete. You can now prepare your Spotlight profile for Pinpoint review.',
            default => 'Your audit status has been updated.',
        };

        $ctaText = match ($this->auditStatus) {
            'needs_info' => 'Go to Messages',
            'complete' => 'Prepare Spotlight Profile',
            default => null,
        };

        $ctaUrl = match ($this->auditStatus) {
            'needs_info' => url('/founder/messages'),
            'complete' => url('/founder/spotlight'),
            default => null,
        };
