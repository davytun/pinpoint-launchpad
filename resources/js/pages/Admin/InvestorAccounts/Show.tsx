...
function KycStatusBadge({ status }: { status: KycStatus }) {
    switch (status) {
        ...
        case 'pending':
            return (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-amber-200/70 bg-amber-50 px-3 py-1 text-xs font-bold text-amber-700">
        ...
        case 'rejected':
            return (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-rose-200/70 bg-rose-50 px-3 py-1 text-xs font-bold text-rose-700">
        ...
        default:
            return (
                <span className="inline-flex items-center gap-1.5 rounded-full border border-zinc-200 bg-zinc-100 px-3 py-1 text-xs font-medium text-zinc-500">
            ...
            <div className="flex h-full max-h-full min-w-0 flex-1 flex-col overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.03)] lg:rounded-[22px]">
                ...
                <div className="flex shrink-0 flex-col justify-between gap-4 border-b border-zinc-100 bg-white px-6 py-5 sm:flex-row sm:items-center lg:px-8">
                    <div className="flex min-w-0 items-center gap-4">
                        ...
                        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-zinc-200 bg-zinc-100 text-xs font-semibold text-zinc-700">
                    ...
                    {canReviewKyc && latestSubmission && latestSubmission.status === 'pending' && (
                        <div className="flex shrink-0 items-center gap-2">
                            <button
                                ...
                                className="flex items-center gap-1.5 rounded-xl bg-zinc-950 px-4 py-2 text-xs font-semibold text-white shadow-2xs transition-all hover:bg-zinc-800"
                            >
                            ...
                            <button
                                ...
                                disabled={form.processing}
                                className="flex items-center gap-1.5 rounded-xl border border-zinc-200/90 bg-white px-3.5 py-2 text-xs font-semibold text-zinc-800 shadow-2xs transition-all hover:bg-zinc-50"
                            >
                ...
                <div className="min-h-0 flex-1 space-y-6 overflow-y-auto p-6 lg:p-8">
                    ...
                    <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                        ...
                        <div className="rounded-xl border border-zinc-200/80 bg-zinc-50/50 p-4">
                            <span className="block text-[11px] font-medium text-zinc-500">KYC Status</span>
                            <p className="mt-1 text-sm font-semibold text-zinc-950">{humanize(investor.kyc_status)}</p>
                        </div>

                        <div className="rounded-xl border border-zinc-200/80 bg-zinc-50/50 p-4">
                            <span className="block text-[11px] font-medium text-zinc-500">Account Created</span>
                    ...
                    <div className="space-y-6 rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-2xs">
                        {/* Profile Details */}
                        <div>
                            <h2 className="mb-3 text-xs font-semibold text-zinc-950">Identity & Contact</h2>
                            <div className="divide-y divide-zinc-100 text-xs">
                                ...
                                <div className="flex items-center justify-between py-2.5">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <a href={`mailto:${investor.email}`} className="ml-0.5 text-zinc-400 hover:text-zinc-800">
                                ...
                                {investor.profile?.address && (
                                    <div className="flex items-center justify-between py-2.5">
                                        ...
                                        <span className="max-w-sm truncate text-right font-medium text-zinc-900">{investor.profile.address}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="border-t border-zinc-100 pt-4">
                            <h2 className="mb-3 text-xs font-semibold text-zinc-950">KYC Submissions & Documents</h2>
                            ...
                            {investor.kyc_submissions && investor.kyc_submissions.length > 0 ? (
                                <div className="space-y-4">
                                    {investor.kyc_submissions.map((sub) => {
                                        ...
                                        return (
                                            <div key={sub.id} className="space-y-4 rounded-2xl border border-zinc-200/90 bg-[#FAFBFD] p-5 shadow-2xs">
                                                <div className="flex items-center justify-between">
                                                    <div className="flex min-w-0 items-center gap-2.5">
                                                        ...
                                                        <div className="min-w-0">
                                                            <p className="truncate text-xs font-semibold text-zinc-950">{sub.original_name}</p>
                                ...
                                <div className="rounded-xl border border-dashed border-zinc-200 p-6 text-center">
                                    <p className="text-xs font-medium text-zinc-400">No KYC submissions recorded yet.</p>
                                </div>
                            )}
                        </div>
                        ...
                        {isRejecting && latestSubmission && (
                            <div className="animate-in fade-in border-t border-zinc-100 pt-4 duration-150">
                                <div className="space-y-3 rounded-xl border border-rose-200 bg-rose-50/40 p-4 text-xs">
                                    <span className="block font-semibold text-rose-900">Reason for KYC Rejection</span>
                                    <textarea
                                        rows={3}
                                        ...
                                        className="w-full resize-none rounded-xl border border-rose-200 bg-white p-3 text-xs text-zinc-900 placeholder:text-zinc-400 focus:border-rose-400 focus:outline-none"
                                    />
                                    <div className="flex items-center justify-end gap-2">
                                        ...
                                        <button
                                            ...
                                            className="rounded-lg bg-rose-600 px-4 py-1.5 text-xs font-semibold text-white shadow-2xs hover:bg-rose-700 disabled:opacity-50"
                                        >
                                            Confirm Rejection
                                        </button>
                                    </div>
