    ...
    function markDeckReviewed() {
        ...
        router.patch(
            ...
            { mark_deck_reviewed: true },
            {
                onSuccess: () => {
                    const remainingRequirements = profile.publish_requirements.filter(
                        (r) => !r.toLowerCase().includes('pitch deck')
                    );
                    onUpdateProfile({
                        ...profile,
                        has_reviewed_pitch_deck: true,
                        pitch_deck: profile.pitch_deck
                            ? { ...profile.pitch_deck, is_reviewed: true }
                            : null,
                        publish_requirements: remainingRequirements,
                        is_ready: remainingRequirements.length === 0,
                    });
            ...
            <div className="relative z-10 w-full max-w-xl bg-white h-full shadow-2xl flex flex-col justify-between overflow-hidden border-l border-zinc-200 animate-in slide-in-from-right duration-200">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 shrink-0 bg-white">
                    <div className="flex items-center gap-3 min-w-0">
                        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-zinc-950 text-white text-xs font-bold shadow-xs">
                        ...
                        <div className="min-w-0">
                            <div className="flex items-center gap-2">
                                ...
                                {profile.is_published ? (
                                    <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 border border-emerald-200/80 px-2 py-0.5 text-[10.5px] font-semibold text-emerald-700">
                                        <span className="size-1.5 rounded-full bg-emerald-500" />
                                        <span>Live in Spotlight</span>
                                    </span>
                                ) : (
                                    <span className="inline-flex items-center rounded-full bg-zinc-100 border border-zinc-200 px-2 py-0.5 text-[10.5px] font-medium text-zinc-500">
                                        Draft
                                    </span>
                                )}
                            </div>
                ...
                <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar p-6 space-y-6">
                    ...
                    {!profile.is_published && profile.publish_requirements.length > 0 && (
                        <div className="rounded-xl border border-amber-200 bg-amber-50/70 p-3.5 space-y-2 text-xs">
                            ...
                            <ul className="space-y-1 text-amber-800 pl-5 list-disc text-[11.5px]">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between">
                                ...
                                <span className={cn('text-[10.5px] font-mono', oneLiner.length > 110 ? 'text-amber-600 font-bold' : 'text-zinc-400')}>
                                    {oneLiner.length} / 120
                                </span>
                            </div>
                            <input
                                ...
                                className="w-full rounded-xl border border-zinc-200/90 bg-white px-3.5 py-2 text-xs text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-400 focus:outline-none shadow-2xs transition-colors"
                            />
                            {errors.spotlight_one_liner && (
                                <p className="text-xs text-rose-600 font-medium">{errors.spotlight_one_liner}</p>
                            )}
                        </div>
                        ...
                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between">
                                ...
                                <span className={cn('text-[10.5px] font-mono', summary.length > 450 ? 'text-amber-600 font-bold' : 'text-zinc-400')}>
                                    {summary.length} / 500
                                </span>
                            </div>
                            <textarea
                                ...
                                className="w-full rounded-xl border border-zinc-200/90 bg-white p-3 text-xs text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-400 focus:outline-none resize-none shadow-2xs transition-colors leading-relaxed"
                            />
                            {errors.spotlight_summary && (
                                <p className="text-xs text-rose-600 font-medium">{errors.spotlight_summary}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="space-y-3 pt-2">
                        <div className="flex items-center justify-between border-b border-zinc-100 pb-2">
                            <h4 className="text-xs font-bold text-zinc-950 uppercase tracking-wider">
                                Spotlight Pitch Deck
                            </h4>
                            <span className="text-[11px] text-zinc-400">Required for investor portal</span>
                        </div>

                        {profile.pitch_deck ? (
                            <div className="rounded-xl border border-zinc-200/80 bg-[#FAFBFD] p-3.5 space-y-3">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2.5 min-w-0">
                                        ...
                                        <div className="min-w-0">
                                            <p className="truncate text-xs font-semibold text-zinc-950">
                                                {profile.pitch_deck.original_filename}
                                            </p>
                                            <p className="text-[11px] text-zinc-400">
                                                {formatBytes(profile.pitch_deck.file_size)}
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div className="flex items-center justify-between pt-1 border-t border-zinc-100 text-xs">
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-200/90 bg-white px-3 py-1.5 text-xs font-semibold text-zinc-800 hover:bg-zinc-50 shadow-2xs transition-colors"
                                    >
                                    ...
                                    {!profile.pitch_deck.is_reviewed && (
                                        <button
                                            ...
                                            className="inline-flex items-center gap-1 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-800 hover:bg-emerald-100 transition-colors shadow-2xs"
                                        >
                                            {reviewingDeck ? (
                            ...
                            <div className="rounded-xl border border-dashed border-zinc-200 p-4 text-center">
                                <p className="text-xs text-zinc-400 font-medium">No spotlight pitch deck uploaded.</p>
                            </div>
                        )}
                    </div>
                        ...
                        {profile.badges.length > 0 ? (
                            <div className="flex flex-wrap gap-1.5">
                                {profile.badges.map((b) => (
                                    <span
                                        key={b.id}
                                        className="inline-flex items-center gap-1 rounded-lg bg-zinc-100 border border-zinc-200/70 px-2.5 py-1 text-[11px] font-medium text-zinc-700"
                                    >
                ...
                <div className="flex items-center justify-between gap-3 px-6 py-3.5 border-t border-zinc-100 bg-[#FAFBFD] shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            onClick={() => saveCopy()}
                            disabled={saving}
                            className="flex items-center gap-1.5 rounded-full border border-zinc-200 bg-white px-3.5 py-1.5 text-xs font-semibold text-zinc-800 hover:bg-zinc-50 shadow-2xs transition-all disabled:opacity-50"
                        >
                            {saving ? (
                            ...
                            <span>Save Copy</span>
                            <kbd className="rounded bg-zinc-100 px-1 py-0.2 text-[10px] font-mono text-zinc-400">⌘↵</kbd>
                        </button>
                        ...
                        {profile.is_published ? (
                            <button
                                ...
                                disabled={publishing}
                                className="flex items-center gap-1.5 rounded-full border border-rose-200 bg-rose-50 px-3.5 py-1.5 text-xs font-semibold text-rose-700 hover:bg-rose-100 shadow-2xs transition-all"
                            >
                                {publishing ? (
                                    <Icon icon="solar:refresh-linear" className="size-3.5 animate-spin" />
                                ) : (
                            ...
                            <button
                                ...
                                className={cn(
                                    'flex items-center gap-1.5 rounded-full px-4 py-1.5 text-xs font-semibold transition-all shadow-2xs',
                                    canPublish
...
export default function SpotlightIndex({
    ...
    return (
        <AdminLayout>
            ...
            <div className="flex flex-1 min-w-0 h-full max-h-full flex-col bg-white rounded-2xl lg:rounded-[22px] border border-zinc-200/80 shadow-xs overflow-hidden p-5 lg:p-7">
                {/* ── Top Bar ─────────────────────────────────────────────────── */}
                ...
                <div className="flex-1 min-h-0 flex flex-col justify-between overflow-hidden">
                                ...
                                >
                                    ...
                                    <div className="min-w-0 flex-1 flex items-center gap-3">
                                        ...
                                        <div className="min-w-0 pr-2">
                                            <span className="font-semibold text-zinc-900 group-hover:underline text-[13px] block truncate">
                                                {p.company_name}
                                            </span>
                                            <span className="text-zinc-400 font-normal text-[11.5px] truncate block">
                                    ...
                                    <div className="w-36 shrink-0">
                                        <span className="font-medium text-zinc-800 truncate block text-[12px]">
                                            {p.sector}
                                        </span>
                                        {p.batch && (
                                    ...
                                    <div className="w-28 shrink-0">
                                        ...
                                        <span className="text-[10.5px] text-zinc-400 font-normal block">
                                    ...
                                    <div className="w-32 shrink-0">
                                        {p.pitch_deck ? (
                                            p.has_reviewed_pitch_deck ? (
                                                <span className="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-700 bg-emerald-50 border border-emerald-200/70 rounded-full px-2.5 py-0.5">
                                                    <Icon icon="solar:check-circle-linear" className="size-3 text-emerald-600" />
                                                    <span>Reviewed</span>
                                                </span>
                                            ) : (
                                                <span className="inline-flex items-center gap-1 text-[11px] font-medium text-amber-700 bg-amber-50 border border-amber-200/70 rounded-full px-2.5 py-0.5">
                                                    <Icon icon="solar:clock-circle-linear" className="size-3 text-amber-600" />
                                    ...
                                    <div className="w-28 shrink-0">
                                        {p.is_published ? (
                                            <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 border border-emerald-200/70 px-2.5 py-0.5 text-xs font-semibold text-emerald-700">
                                                <span className="size-1.5 rounded-full bg-emerald-500" />
                                                <span>Published</span>
                                            </span>
                                        ) : (
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-zinc-100 text-xs text-zinc-400 shrink-0">
                        ...
                        <p className="font-medium text-zinc-600">
                            {totals?.published ?? 0} live on Investor Portal
                        </p>
                    </div>
                </div>
