    ...
    return (
        <div
            className={cn(
                ...
                msg.type === 'success' ? 'border-emerald-500/25 bg-emerald-50 text-emerald-800' : 'border-rose-500/25 bg-rose-50 text-rose-800',
            )}
        >
            <div className="flex items-center gap-2">
                <Icon icon={msg.type === 'success' ? 'solar:check-circle-linear' : 'solar:danger-circle-linear'} className="size-4 shrink-0" />
                <span>{msg.text}</span>
            </div>
...
function SortTh({
    ...
    return (
        <th className={cn('px-5 py-3', className)}>
            ...
            >
                <span className={cn(active && 'font-extrabold text-zinc-950')}>{label}</span>
                <Icon
                    icon={active ? (dir === 'asc' ? 'solar:arrow-up-linear' : 'solar:arrow-down-linear') : 'solar:sort-vertical-linear'}
...
function DeleteModal({ entry, isOpen, onClose }: { entry: WaitlistEntry | null; isOpen: boolean; onClose: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            {/* Backdrop */}
            <div className="animate-in fade-in-0 fixed inset-0 bg-zinc-950/40 backdrop-blur-xs transition-opacity duration-200" onClick={onClose} />

            {/* Modal Card */}
            <div className="animate-in zoom-in-95 relative w-full max-w-md overflow-hidden rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-2xl duration-200">
                <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-[13px] leading-relaxed font-normal text-zinc-500">
                            Are you sure you want to remove <span className="font-semibold text-zinc-900">{entry.name}</span> ({entry.email}) from the
                ...
                <div className="mt-6 flex items-center justify-end gap-2.5 border-t border-zinc-100 pt-4">
                    <button
                        ...
                        onClick={onClose}
                        disabled={isDeleting}
                        className="rounded-xl border border-zinc-200/80 bg-white px-4 py-2 text-xs font-semibold text-zinc-700 shadow-2xs transition-colors hover:bg-zinc-50"
                    >
                        Cancel
                    </button>
                    ...
                    <button
                        ...
                        disabled={isDeleting}
                        className="flex items-center gap-1.5 rounded-xl bg-rose-600 px-4 py-2 text-xs font-bold text-white shadow-2xs transition-colors hover:bg-rose-700 disabled:opacity-50"
                    >
...
function RowActions({ entry, onRequestDelete }: { entry: WaitlistEntry; onRequestDelete: (entry: WaitlistEntry) => void }) {
    ...
    return (
        <div className="flex items-center justify-end gap-1 opacity-70 transition-opacity group-hover:opacity-100">
            ...
            <button
                ...
                className="flex h-7 w-7 items-center justify-center rounded-lg border border-zinc-200/70 bg-white text-zinc-400 transition-all hover:bg-zinc-100 hover:text-zinc-800 disabled:opacity-40"
            >
            ...
            <button
                ...
                title="Remove lead"
                className="flex h-7 w-7 items-center justify-center rounded-lg border border-zinc-200/70 bg-white text-zinc-400 transition-all hover:border-rose-200 hover:bg-rose-50 hover:text-rose-600"
            >
    ...
    return (
        <AdminLayout>
            ...
            <div className="flex h-full max-h-full min-w-0 flex-1 flex-col overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.03)] lg:rounded-[22px]">
                ...
                <div className="flex shrink-0 flex-col justify-between gap-4 border-b border-zinc-100 bg-white px-6 py-4 sm:flex-row sm:items-center">
                    ...
                    <div className="flex items-center gap-2">
                        <a
                            ...
                            className="flex items-center gap-2 rounded-xl border border-zinc-200/80 bg-white px-3.5 py-1.5 text-xs font-semibold text-zinc-800 shadow-2xs transition-colors hover:border-zinc-300 hover:bg-zinc-50"
                        >
                ...
                <div className="grid shrink-0 grid-cols-2 divide-x divide-zinc-100 border-b border-zinc-100 bg-[#FAFBFD] sm:grid-cols-5">
                    <div className="px-6 py-3">
                        <span className="text-[10px] font-bold tracking-wider text-zinc-400 uppercase">Total Leads</span>
                    ...
                    <div className="px-6 py-3">
                        <span className="text-[10px] font-bold tracking-wider text-zinc-400 uppercase">Founders</span>
                        <div className="mt-0.5 flex items-baseline gap-2">
                            <span className="text-[17px] font-bold text-zinc-950 tabular-nums">{totals.founder}</span>
                            <span className="text-[11px] font-medium text-zinc-400">{pct(totals.founder, totals.all)}</span>
                        </div>
                    </div>

                    <div className="px-6 py-3">
                        <span className="text-[10px] font-bold tracking-wider text-zinc-400 uppercase">Investors</span>
                        <div className="mt-0.5 flex items-baseline gap-2">
                            <span className="text-[17px] font-bold text-zinc-950 tabular-nums">{totals.investor}</span>
                            <span className="text-[11px] font-medium text-zinc-400">{pct(totals.investor, totals.all)}</span>
                        </div>
                    </div>

                    <div className="px-6 py-3">
                        <span className="text-[10px] font-bold tracking-wider text-zinc-400 uppercase">Emails Sent</span>
                        <div className="mt-0.5 flex items-baseline gap-2">
                            <span className="text-[17px] font-bold text-zinc-950 tabular-nums">{totals.email_sent}</span>
                            <span className="text-[11px] font-semibold text-emerald-600">{pct(totals.email_sent, totals.all)}</span>
                        </div>
                    </div>

                    <div className="px-6 py-3">
                        <span className="text-[10px] font-bold tracking-wider text-zinc-400 uppercase">Converted</span>
                        <div className="mt-0.5 flex items-baseline gap-2">
                            <span className="text-[17px] font-bold text-zinc-950 tabular-nums">{totals.converted}</span>
                            <span className="text-[11px] font-semibold text-emerald-600">{pct(totals.converted, totals.all)}</span>
                        </div>
                    </div>
                </div>
                        ...
                        {(
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span
                                        className={cn(
                                            'py-0.2 rounded-full px-1.5 text-[10px] font-bold tabular-nums',
                        ...
                        <table className="w-full border-collapse text-left text-xs">
                            <thead className="sticky top-0 z-10 border-b border-zinc-200/80 bg-zinc-50/95 backdrop-blur-xs">
                                <tr>
                                    ...
                                    <th className="px-5 py-3 text-[11px] font-bold tracking-wider text-zinc-400 uppercase">Organization</th>
                                    ...
                                    <th className="w-25 px-5 py-3 text-right text-[11px] font-bold tracking-wider text-zinc-400 uppercase">
                                        Actions
                                    </th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-zinc-100 bg-white">
                                {entries.data.map((entry) => {
                                    ...
                                    return (
                                        <tr key={entry.id} className="group transition-colors duration-150 hover:bg-[#F9F9FB]">
                                            ...
                                            <td className="px-5 py-3.5">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="truncate text-[11.5px] font-normal text-zinc-400">{entry.email}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            {/* Signed Up */}
                                            <td className="px-5 py-3.5 text-[12px] font-normal text-zinc-500 tabular-nums">
                                            ...
                                            <td className="px-5 py-3.5">
                                                {emailSent ? (
                                                    ...
                                                    <span className="inline-flex items-center gap-1.5 text-[12px] font-normal text-zinc-400">
                                            ...
                                            {/* Actions Toolbar */}
                                            <td className="px-5 py-3.5">
                                                <RowActions entry={entry} onRequestDelete={(e) => setDeletingEntry(e)} />
                                            </td>
                                        </tr>
                                    );
                                })}
                    ...
                    <div className="flex shrink-0 items-center justify-between border-t border-zinc-100 bg-white px-6 py-3">
                        <p className="text-xs text-zinc-500">
                            ...
                            <span className="font-bold text-zinc-900">
                                {(entries.current_page - 1) * entries.per_page + 1}–{Math.min(entries.current_page * entries.per_page, entries.total)}
                            </span>{' '}
                                ...
                                {entries.links.map((link, i) => {
                                    ...
                                    return link.url ? (
                                        <Link
                                            ...
                                            preserveScroll
                                            className={cn(
                                                'flex h-7.5 min-w-7.5 items-center justify-center rounded-lg px-2 text-xs font-semibold shadow-2xs transition-colors',
                                                link.active
            ...
            <DeleteModal entry={deletingEntry} isOpen={deletingEntry !== null} onClose={() => setDeletingEntry(null)} />
        </AdminLayout>
    );
}
