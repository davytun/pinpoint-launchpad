...
export default function InvestorLandingHeader({ cta }: Props) {
    ...
    return (
        <>
            <div className="fixed top-4 left-1/2 z-50 w-full max-w-5xl -translate-x-1/2 px-4 font-sans sm:px-6">
                ...
                {mobileOpen && (
                    <div className="absolute top-18 right-4 left-4 z-50 flex flex-col gap-3 rounded-3xl border border-zinc-200/50 bg-white/95 px-6 py-5 shadow-xl backdrop-blur-lg md:hidden">
                        ...
                        <Link
                            ...
                            className="rounded-full border border-zinc-200 py-2.5 text-center text-sm font-bold text-zinc-700"
                        >
                            Log in
                        </Link>
                        {cta.enabled ? (
            ...
            <div className="h-[4.5rem] shrink-0 md:h-20" aria-hidden="true" />
        </>
    );
}
